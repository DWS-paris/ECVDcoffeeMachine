...
*/
  // Réseau local
  const char *ssid = "WEMOSnetwork";
  const char *password = "THXtimothy";
  
  // Nom de domaine du serveur : fini par .local
  const char* mdnsName = "dwswemos";
/**/
...
*/
  ...
  const int coffeePin = 12; /*D6*/
  const int sugarPin = 13; /*D7*/
  const int waterPin = 2; /*D4*/
//
...
*/
  void startWiFi() {
    ...
    Serial.print("Création de l'Access Point à l'adresse : \"");
    Serial.println(ssid);

    // Ajouter une connexion au réseau distant (plusieurs connexions possible)
    wifiMulti.addAP("freebox_jul", "180879mdpjuju");

    // Connexion au-x réseau-x
